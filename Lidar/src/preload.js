...
contextBridge.exposeInMainWorld('lidar', {
  ...
  // Persistance fichier (survit aux crashs et redémarrages)
  saveSettings:  (s)          => ipcRenderer.invoke('lidar:save-settings', s),
  loadSettings:  ()           => ipcRenderer.invoke('lidar:load-settings'),

  // OSC
  setOscConfig:  (cfg)        => ipcRenderer.invoke('lidar:osc-config', cfg),
  sendBlobs:     (blobs)      => ipcRenderer.send('lidar:blobs', blobs),
